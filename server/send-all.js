...
async function sendMd(token, touser, content) {
  const res = await fetch(
...
async function main() {
  ...
  for (const name of names) {
    ...
    if (!user) { console.log(`${name}: 未绑定`); continue; }

    try {
      await sendMd(token, user.wechatUserId, PART1);
      await sendMd(token, user.wechatUserId, PART2);
      console.log(`${name}: 两条均发送成功 √`);
    } catch (err) {
