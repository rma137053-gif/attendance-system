...
const envContent = fs.readFileSync('/app/server/.env', 'utf8');
const env = {};
...
async function sendMarkdown(token, agentId, touser, content) {
...
async function main() {
  ...
  // Step 1: List WeChat users
  const wxUsers = await listWxUsers(token);
  console.log('企微通讯录:', wxUsers.map(u => u.name).join(', '));

  // Step 2: Sync local users with WeChat users
  const localUsers = await prisma.user.findMany({
    where: { status: 'ACTIVE', wechatUserId: null },
    select: { id: true, name: true },
  });

  for (const local of localUsers) {
    // Exact match
    let wx = wxUsers.find(w => w.name === local.name);
    // Fuzzy: local name contained in wx name, or wx name contained in local name
    if (!wx) {
      wx = wxUsers.find(w => {
        const wClean = w.name.replace(/\s/g, '');
        const lClean = local.name.replace(/\s/g, '');
        return wClean.includes(lClean) || lClean.includes(wClean);
      });
    }
    if (wx) {
      ...
      console.log(`已绑定: ${local.name} -> ${wx.userid} (企微名: ${wx.name})`);
    }
  }

  // Step 3: Send proposal to target users
  const targets = ['麻建平', '姚海霞'];
  console.log('\n=== 发送提案 ===');
  for (const name of targets) {
    ...
    if (!user || !user.wechatUserId) {
      console.log(`${name}: 仍未绑定，跳过`);
      continue;
    }
    try {
      await sendMarkdown(token, env.WECHAT_AGENT_ID, user.wechatUserId, PROPOSAL);
      console.log(`${name} (${user.wechatUserId}): 发送成功 √`);
    } catch (err) {
      console.log(`${name} (${user.wechatUserId}): 发送失败 - ${err.message}`);
    }
  }
