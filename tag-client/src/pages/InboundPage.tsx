...
export default function InboundPage() {
  ...
  if (user?.role !== 'ADMIN') {
    return <div className="text-center py-16 text-gray-400">仅管理员可操作入库</div>;
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await api.post('/transactions/in', { quantity: Number(quantity), note });
      showToast(`入库 ${quantity} 张成功`, 'success');
      navigate('/');
    } catch (err: any) {
  ...
  return (
    <div className="max-w-sm mx-auto">
      <h1 className="text-base font-bold text-gray-800 mb-4">📥 吊牌入库</h1>
